...
import 'package:story_roles_web/presentation/screens/projects/widgets/project_tile_list.dart';
...
class _ProjectsViewState extends State<ProjectsView> {
  ...
  Future<void> _showCreateDialog(BuildContext context) async {
    ...
    if (result != null && result.trim().isNotEmpty && context.mounted) {
      context.read<ProjectsBloc>().add(CreateProjectEvent(name: result.trim()));
    }
  }
    ...
    return BlocBuilder<ProjectsBloc, ProjectsState>(
      builder: (blocCtx, state) {
        if (state.status == ProjectsBlocStatus.loading ||
            state.status == ProjectsBlocStatus.initial) {
          return const Center(
            child: CircularProgressIndicator(color: Color(0xFFFF8A5B)),
          );
        }
        if (state.status == ProjectsBlocStatus.failure) {
          return Center(
            child: Column(
              ...
              children: [
                ...
                TextButton(
                  onPressed: () =>
                      blocCtx.read<ProjectsBloc>().add(LoadProjectsEvent()),
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        return RepaintBoundary(
                          child: ProjectListTile(
                            ...
                            onRename: () =>
                                _showRenameDialog(blocCtx, project),
                            onDelete: () =>
                                _showDeleteDialog(blocCtx, project),
                          ),
                        );
                      },
                    ),
